...
const DEFAULT_ORIGIN = "https://www.ampere.sh";
const DEFAULT_API_BASE = "https://api.ampere.sh";
...
function buildApiRequest(path, token, method = "GET", body = null) {
  const headers = {
  ...
  return { url: `${DEFAULT_API_BASE}${path}`, init };
}
...
export class AmpereClient {
  ...
  async api(path, method = "GET", body = null) {
    ...
    const { url, init } = buildApiRequest(path, token, method, body);
