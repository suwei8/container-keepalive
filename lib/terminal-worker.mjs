...
const DEFAULT_TERMINAL_WS = "wss://api.ampere.sh/api/my/terminal";
...
async function main() {
  ...
  const { token, commands, timeoutMs, cols, rows } = payload;
  const waitFor = reviveWaitFor(payload.waitFor);
  const ws = new WebSocket(`${DEFAULT_TERMINAL_WS}?token=${encodeURIComponent(token)}`);
